...
- `doc/` — two markdown guides. **`ARCHITECTURE.md` is the system-level reference** (topology, data flow, the CLI's internal design decisions) — read it before making non-trivial changes to `src/cli/`. **`CLUSTER_SETUP.md`** is the step-by-step setup doc — single Mac through the full two-Mac cluster build, every command verified, ending in a go-to command cheatsheet grouped by task.
- `src/` — all code: `src/tools/` (`mlxctl`; the distributed-MLX benchmark script, the zero-dep chat client, example configs, `requirements*.txt`), `src/cli/` (the TypeScript chat client, including its built-in `/agent` coding agent).
...
- `defaultMode` (`"server"` | `"solo"`) picks the startup mode. **This machine's config is `"solo"`**: serve on your Mac immediately, skipping the M1 probe and the wear-leveling check. `"server"` (the shipped example default) is Pattern A with local fallback. A deliberate solo session shows `solo · your Mac`; the emergency fallback shows `solo · your Mac (server unreachable)`.
- `/mode ollama [<model>]` serves through a local Ollama daemon, making `ollama pull`-ed `-mlx` models usable without re-downloading them into the HF cache (Ollama has its own store). It attaches to a running daemon and never stops one it didn't start. `/model` then lists Ollama's models, not the HF cache. See `doc/ARCHITECTURE.md`'s "Ollama backend".
