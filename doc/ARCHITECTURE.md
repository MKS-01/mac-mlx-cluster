...
[`CLUSTER_SETUP.md`](./CLUSTER_SETUP.md). The external coding-agent harness
(OpenCode + local Qwen) that runs against this stack is described in this
file's [Coding-agent harness](#coding-agent-harness-opencode--local-qwen)
section below.
...
flowchart LR
    ...
    you --> hx["harness → OpenCode<br/>worker + @evaluator"]
    you --> ctl["mlxctl"]
...
buys over the external OpenCode harness described next, which remains the
